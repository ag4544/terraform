...
resource "local_file" "vps" {
  content = templatefile("${path.module}/vpssummary.tpl", {
    server_fqdn = aws_route53_record.task11.*.name,
    server_ip   = hcloud_server.terraform_task11.*.ipv4_address,
  rootpass = random_password.password.*.result })
